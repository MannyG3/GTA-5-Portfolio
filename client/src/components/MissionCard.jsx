    ...
    >
      <Link to={`/projects/${slug}`}>
        ...
        <div className="relative h-48 overflow-hidden bg-gta-bg">
          ...
          {/* Featured badge */}
          {featured && (
            <div className="absolute top-3 left-3 px-3 py-1 font-rajdhani font-bold text-xs uppercase tracking-wider"
                 style={{ 
                   background: 'linear-gradient(135deg, #FFB35C 0%, #FF7A59 100%)',
                   color: '#0B0D10'
                 }}>
              Featured
            </div>
          ...
          {/* Status badge */}
          {status === 'in-progress' && (
            <div className="absolute top-3 right-3 px-3 py-1 bg-gta-teal/90 text-gta-bg font-rajdhani font-bold text-xs uppercase tracking-wider">
              In Progress
            </div>
          )}
